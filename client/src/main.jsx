...
function Login({onLogin}){
 ...
 async function submit(e){e.preventDefault();setBusy(true);setError('');try{const d=await api('/auth/login',{method:'POST',body:JSON.stringify({email,password})});localStorage.setItem('token',d.token);onLogin(d.user)}catch(e){setError(e.message)}finally{setBusy(false)}}
 return <main className="center"><form className="card auth" onSubmit={submit}><div className="logo">🛠️</div><h1>IT Service Desk</h1><p>Sign in to manage support tickets.</p><input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} required/><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required/><button disabled={busy}>{busy?'Signing in...':'Sign in'}</button>{error&&<div className="error">{error}</div>}</form></main>
}
